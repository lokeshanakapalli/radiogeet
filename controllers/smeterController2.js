import SpeedMeter2 from '../models/smeterModel2.js'; // Adjust the import path as necessary
...
export const createSpeedMeterWidget = async (req, res) => {
  ...
  try {
    const newSpeedMeterWidget = new SpeedMeter2({
      dashboardId,
      widgetType: "speedmeter2",
      speedMeter,
      topic,
      min, // Optional field
      max, // Optional field
...
export const getSpeedMeterWidgets = async (req, res) => {
  try {
    // Retrieve all SpeedMeter widgets
    const speedMeterWidgets = await SpeedMeter2.find().exec();
...
export const updateSpeedMeterWidget = async (req, res) => {
  ...
  try {
    const updatedSpeedMeterWidget = await SpeedMeter2.findByIdAndUpdate(
      id,
...
export const deleteSpeedMeterWidget = async (req, res) => {
  ...
  try {
    const deletedSpeedMeterWidget = await SpeedMeter2.findByIdAndDelete(id);
