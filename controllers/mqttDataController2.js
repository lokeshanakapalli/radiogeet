...
// Get the last 2000 MQTT data entries
export const getLast2000MqttData = async (req, res) => {
  try {
    const data = await MqttData.find().sort({ createdAt: -1 }).limit(10);
    res.status(200).json(data);
  } catch (error) {
    res.status(500).json({ message: "Error retrieving last 2000 MQTT data", error });
  }
};
